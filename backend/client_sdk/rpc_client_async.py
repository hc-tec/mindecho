...
class EAIRPCClient:
    ...
    async def start(self):
        ...
        logger.info(
            f"EAIRPCClient started. Webhook receiver listening on http://{self.webhook_host}:{self.webhook_port}/webhook")
    ...
    async def _rpc_call(self, plugin_id: str, params: Dict[str, Any], timeout_sec: float):
        ...
        try:
            ...
            await self._http_client.post("/api/v1/topics", json={"topic_id": topic_id, "name": topic_id,
                                                                 "description": f"RPC for {plugin_id}"})
            await self._http_client.post(f"/api/v1/topics/{topic_id}/subscriptions",
            ...
            # Execute plugin
            await self._http_client.post("/api/v1/tasks",
                                         json={"plugin_id": plugin_id, "run_mode": "once", "params": params,
                                               "topic_id": topic_id})
    ...
    def _merge_params(self, task_params, service_params, extra_params):
        return {k: v for k, v in {**task_params.__dict__, **service_params.__dict__, **extra_params}.items() if
                v is not None}

    # --- Public RPC Methods ---
    async def get_collection_list_from_bilibili(self, user_id: Optional[str] = None, rpc_timeout_sec=60,
    ...
    async def get_collection_list_videos_from_bilibili(self, collection_id: str, user_id: Optional[str] = None,
                                                       rpc_timeout_sec=120, task_params: TaskParams = TaskParams(),
                                                       service_params: ServiceParams = ServiceParams()):
    ...
    async def get_video_details_from_bilibili(self, bvid: str, rpc_timeout_sec=60,
                                              task_params: TaskParams = TaskParams(),
                                              service_params: ServiceParams = ServiceParams()):
    ...
    async def get_collection_list_from_xiaohongshu(self, user_id: Optional[str] = None, rpc_timeout_sec=60,
                                                   task_params: TaskParams = TaskParams(),
                                                   service_params: ServiceParams = ServiceParams()):
        params = self._merge_params(task_params, service_params, {"user_id": user_id})
    ...
    async def get_collection_favorite_items_from_xiaohongshu(self, collection_id: str, user_id: Optional[str] = None,
                                                             rpc_timeout_sec=120,
                                                             task_params: TaskParams = TaskParams(),
                                                             service_params: ServiceParams = ServiceParams()):
        params = self._merge_params(task_params, service_params, {"collection_id": collection_id, "user_id": user_id})
    ...
    async def get_note_details_from_xiaohongshu(
        ...
        """Get detailed information for a single Xiaohongshu note.

        Args:
            note_id: Note ID
            xsec_token: Note's xsec_token
            wait_time_sec: Page loading wait time in seconds (default: 3)
            rpc_timeout_sec: RPC timeout in seconds
            task_params: Task parameters (cookies, etc.)
            service_params: Service parameters

        Returns:
            Dict containing note details (title, desc, author, tags, images, video, etc.)
        """
